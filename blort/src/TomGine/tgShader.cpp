...
tgShader::tgShader(const char *vertex_file, const char *fragment_file, const char *header)
{
    GLint status;
	...
	if( vertex_file && !vs ){
		sprintf(charbuffer, "[tgShader::tgShader] Error loading file for vertex shader '%s'.", vertex_file);
		throw std::runtime_error(charbuffer);
	}

	
	if( fragment_file && !fs ){
		sprintf(charbuffer, "[tgShader::tgShader] Error loading file for fragment shader '%s'.", fragment_file);
		throw std::runtime_error(charbuffer);
	}
...
char* TomGine::read_text_file(const char* file)
{
    ...
    const size_t size = fread(retv, sz, 1, f);
    retv[sz] = size;
    retv[sz] = 0;
    fclose(f);
